...
func main() {
	pageURL := "https://keepalive.myedgedemo.com" // Replace with the target URL, plan to make command argument
	csp, err := generateCSP(pageURL)
	if err != nil {
		fmt.Println("Error:", err)
		return
	}
	fmt.Println("Generated CSP Header:")
	fmt.Println(csp)
}
...
/// default-src 'self' *.myedgedemo.com; img-src * https: data:; script-src * https: blob:; style-src * https: blob: 'unsafe-inline'; font-src *;
/// script-src 'self' keepalive.myedgedemo.com cdn.jsdelivr.net; style-src 'self' cdnjs.cloudflare.com; img-src 'self';
